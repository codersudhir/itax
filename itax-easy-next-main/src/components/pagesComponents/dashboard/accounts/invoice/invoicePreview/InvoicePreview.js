    ...
    return (
        <div className="flex gap-2 flex-wrap-reverse  justify-between p-2 container 2xl:max-w-7xl mx-auto">
            <div className=" shadow-inner shadow-neutral-500/50 rounded-md p-2 grid gap-2 grid-cols-2 bg-gray-200">
                <button onClick={()=>setIsEditable(false)} className={`rounded-md ${isEditable?"":"bg-blue-500 text-neutral-50"} grid place-content-center py-1 px-2`} type="button">Preview</button>
                <button onClick={()=>setIsEditable(true)} className={`rounded-md ${isEditable?"bg-blue-500 text-neutral-50":""} grid place-content-center py-1 px-2`} type="button">Edit</button>
            </div>
        </div>
    )
}
export default function InvoicePreview(props) {
	const { respInvoice,respParty,businessProfile } = props;
    ...
    const {handleSubmit,register, watch, setValue}=useForm({
        defaultValues:{
            ...
            Address_DetailsFrom:profile?.address,
            phone_DetailsFrom:"+91 ",
            email_DetailsFrom:profile?.email,
            // 
    ...
    return (
		<div className="bg-gray-100 p-4">
			...
			<div className="mt-4 grid gap-4 xl:grid-cols-[3fr_1fr] mx-auto container 2xl:max-w-7xl">
				<form onSubmit={handleSubmit(onCreatePDF)} className="min-h-full p-4 bg-neutral-50">
					{isEditable ? (
						<EditTemplate
                        ...
                        <>
                            <div className="grid justify-end">
                                <button className="btn-primary ml-auto flex items-center">
                                    <Icon className="text-2xl mr-2" icon="material-symbols:download"/>
                                    PDF
                                </button>
                            </div>
