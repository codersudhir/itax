...
export default function CreateItem() {
    ...
    const onSubmit = async (formData) => {
        if(!itemName && !hsnCode && !taxExempted && !unit && !price && !purchasePrice && !openingStock && !closingStock && !cgst && !sgst && !igst && !utgst && !description && userId){
        ...
        try {
            ...
            const response = await userAxios.post("/invoice/items",JSON.stringify(jsonData));
            if (response.data.success) {
                toast.success("New Item Created");
                router.replace("/dashboard/accounts/invoice/items")
            }
            ...
            <section className="p-4 max-w-6xl mx-auto">
                <form onSubmit={handleSubmit(onSubmit)}>
                    <ul className="grid grid-cols-12 gap-4">
                        ...
                        <li className="md:col-span-6 col-span-12 relative">
                            <label htmlFor="purchasePrice" className={formClassNames.label}>Purchase Price</label>
                            <div className="relative">
                                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center px-2.5 text-gray-500">
                                    ₹
                                </div>
                        ...
                        <li className="col-span-12">
                            <label htmlFor="Description" className={formClassNames.label}>Description</label>
                            <textarea 
                                id="Description" 
                                className={formClassNames.input}
                                rows="3" 
                        ...
                        <li className="md:col-span-6 col-span-12">
                            <div className="flex justify-between md:justify-start items-center">
                                <button type="submit" className="focus:outline-none text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-900">
                                    Submit
                                </button>
                            </div>
                        </li>
