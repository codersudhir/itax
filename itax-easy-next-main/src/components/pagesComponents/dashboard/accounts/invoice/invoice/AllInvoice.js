...
export default  function AllInvoice() {
    ...
    const linkToCreateSale = "/dashboard/accounts/invoice/all-invoice/create-invoice";
                    ...
                    <table className="w-full text-sm text-left rtl:text-right text-neutral-500 dark:text-neutral-400">
                        <thead className=" sticky -top-0 shadow-md text-neutral-700 uppercase bg-neutral-50 dark:bg-neutral-700 dark:text-neutral-400">
                            <tr className="border-b-2 dark:border-neutral-900">
                                {invoiceData.itemsData_title.map((item,index)=>
                        ...
                        <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                            {respData?.items.length != 0 && respData?.items.map((invoice,i) => (
                                <tr key={i} className="odd:bg-white odd:dark:bg-neutral-900 even:bg-neutral-50 even:dark:bg-neutral-800 border-b dark:border-neutral-700">
                                    <td className="px-6 py-4 font-semibold text-gray-900">{invoice.itemName}</td>
                                    <td className="px-6 py-4">{invoice.id}</td>
                                    <td className="px-6 py-4">{invoice.unit}</td>
                                    <td className="px-6 py-4">{invoice.price}</td>
                                    <td className="px-6 py-4">{invoice.openingStock}</td>
                                    <td className="px-6 py-4">{invoice.closingStock}</td>
                                    <td className="px-6 py-4">{invoice.purchasePrice}</td>
                                    <td className="px-6 py-4">{invoice.gst}</td>
                                    <td className="px-6 py-4">{invoice.taxExempted}</td>
                                    <td className="px-6 py-4">{invoice.description}</td>
                                    <td className="px-6 py-4">{invoice.hsnCode}</td>
                                    <td className="px-6 py-4">{invoice.categoryId}</td>
                                    <td className="px-6 py-4">{invoice.supplierId}</td>
                                    <td className="px-6 py-4">{invoice.userId}</td>
                                    ...
                                    <td className="px-6 py-4 flex">
                                        ...
                                        <Icon onClick={()=>handleDelete(item.id)} icon="material-symbols:delete" className=" cursor-pointer hover:text-red-700 text-xl"/>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
