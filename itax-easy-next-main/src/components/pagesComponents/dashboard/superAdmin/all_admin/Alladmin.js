...
const Alladmin = () => {
  ...
  useEffect(() => {
    const getAllUsers = async () => {
      try {
        ...
        const result = resp.data.data.users.filter((word) => word.userType == 'admin');      
        setAdmin(result)
  ...
  return (
    <>
      <DashSection title={"All Admin"} titleRight={<Link href="/dashboard/create-admin"><button className="btn-primary my-2">Create Admin</button></Link>}>
        <div >
