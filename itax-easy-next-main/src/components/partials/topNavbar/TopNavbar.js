        ...
        subLink: [
            ...
            },
            {
                ...
                subLink: [
                    ...
                    {
                        href: "/financialcal/taxcalculator",
                        name: "Tax Calculator",
                    },
                    {
...
function SubLinksItems({linkItem}) {
    ...
    if (subLink) {
        return (
            <li className="group/subLink relative text-nowrap">
                <button className="min-w-56 w-full flex gap-1 items-center tracking-tighter p-2 text-gray-800 group-hover/subLink:text-blue-500 font-semibold text-md">
                    <Icon className="rotate-0 group-hover/subLink:-rotate-90  transition-transform 0.3s" icon="iconamoon:arrow-down-2"/>
                    {name}
                    {upcoming && <span className="ml-auto bg-green-100/50 text-green-500 rounded-sm px-1 py-[2px] text-sm">Upcoming</span>}
                </button>
                <ul className="
    ...
    return (
        <li className="text-nowrap">
            <Link className="min-w-56 flex gap-1 tracking-tighter px-4 py-2 font-semibold text-md  text-gray-800 hover:text-blue-500" href={href}>
                {name}
                {upcoming && <span className="ml-auto bg-green-100/50 text-green-500 rounded-sm px-1 py-[2px] text-sm">Upcoming</span>}
            </Link>
        </li>
...
function LinksItems({linkItem}) {
    ...
    if (subLink) {
        return (
            <li className="group/link relative text-nowrap">
                <button className="capitalize tracking-tighter flex items-center font-semibold text-base py-2 group-hover/link:text-blue-500 border-b-4 border-b-blue-500/0 group-hover/link:border-b-blue-500">
                    <Icon className="rotate-0 group-hover/link:-rotate-90  transition-transform 0.3s" icon="iconamoon:arrow-down-2"/>
                    {name}
                </button>
                <ul className=" 
                group-hover/link:block hidden absolute top-11 left-0 
                bg-white rounded-md shadow-sm shadow-blue-600 border
                ">
                    {subLink.map((link, index) => (
                        <SubLinksItems key={index} linkItem={link}/>
    ...
    return (
        <li className="text-nowrap capitalize">
            <Link className="tracking-tighter block font-semibold text-base py-2 text-gray-800 hover:text-blue-500 border-b-4 border-b-blue-500/0 hover:border-b-blue-500" href={href}>{name}</Link>
        </li>
    );
}
...
export default function TopNavbar({handleSidebar, isSidebarOpen }) {
	...
	return (
		<header className={`${isSidebarOpen ? "md:pl-64":"md:pl-16"} transition-[padding] duration-200 bg-white sticky w-full z-20 top-0 start-0 border-b border-gray-200`}>
			<div className="flex items-center justify-between mx-auto min-h-14 px-4">
                <button onClick={handleSidebar} className="blochref md:hidden px-2 mr-2 text-3xl text-blue-700 hover:scale-105 hover:text-blue-800 transition-transform duration-200">
                    <Icon className="rounded-sm" icon="ant-design:menu-unfold-outlined"/>
                </button>
				<Link href="/" className="min-w-max flex items-center mr-auto" >
					<Image
						width={60}
						height={60}
						src="/logo.svg"
