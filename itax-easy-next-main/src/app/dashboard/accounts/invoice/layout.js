...
function SideBarItem(props) {
        ...
        return (
            <li className="grid gap-2 mt-4">
                <div className="pt-2 uppercase font-light ml-3 flex items-center gap-2 ">
                    <span className="mr-6 bg-neutral-50 text-blue-500 rounded-full text-lg p-1" ><Icon icon={data.iconName}/></span>
                    <span className="text-sm">{data.title}</span>
                </div>
                <ul className="grid gap-2 p-1">
                    {data.subMenuItems.map((item, index) => (
                        <li key={index}>
                            <Link href={item.linkTo} className="ml-2 flex items-center gap-2 font-medium capitalize">
                                <span className="mr-6 bg-neutral-50 text-blue-500 rounded-full text-lg p-1" ><Icon icon={item.iconName}/></span>
    ...
    return (
        <li>
            <Link href={data.linkTo} className="uppercase font-light ml-3 flex items-center gap-2">
                <span className="mr-6 bg-neutral-50 text-blue-500 rounded-full text-lg p-1"><Icon icon={data.iconName}/></span>
                <span className="text-sm">{data.title}</span>
            </Link>
        </li>
        ...
        <>
            ...
            >
                <nav className={`
                        ...
                        `}
                >
                    <SidebarToggle isSidebarOpen={isSidebarOpen} handleSidebar={handleSidebar}/>
                    <ul>
                        {data.map((item, index) => (
